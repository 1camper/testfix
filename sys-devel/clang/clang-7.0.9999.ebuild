...
clang_tools_extra_EGIT_REPO_URI="https://git.llvm.org/git/clang-tools-extra.git
		https://github.com/llvm-mirror/clang-tools-extra.git"
llvm_EGIT_REPO_URI="https://git.llvm.org/git/llvm.git
...
src_unpack() {
	...
	local livevar clang_tools_extra_REPO_URI clang_REPO_URI llvm_REPO_URI

	livevar=clang_tools_extra_LIVE_REPO
	clang_tools_extra_REPO_URI=${!livevar-${clang_tools_extra_EGIT_REPO_URI}}
	livevar=clang_LIVE_REPO
	clang_REPO_URI=${!livevar-${EGIT_REPO_URI}}
	livevar=llvm_LIVE_REPO
	llvm_REPO_URI=${!livevar-${llvm_EGIT_REPO_URI}}

	git-r3_fetch "${clang_tools_extra_REPO_URI}"
	if use test; then
		# needed for patched gtest
		git-r3_fetch "${llvm_REPO_URI}"
	fi
	git-r3_fetch "${clang_REPO_URI}"

	git-r3_checkout "${clang_tools_extra_REPO_URI}" \
		"${S}"/tools/extra
	if use test; then
		git-r3_checkout "${llvm_REPO_URI}" \
			"${WORKDIR}"/llvm '' \
			lib/Testing/Support utils/{lit,llvm-lit,unittest} \
			unittests/unittest.cfg.in
	fi

	git-r3_checkout "${clang_REPO_URI}" "${S}"
}
